// server.js
// YouTube Stream Schedule Sign-Up App
// Deploys on Railway as a single Node/Express app with SQLite persistence.
// Admin login: username "admin", password "madrox79" by default.
// Optional IP auto-admin: set ADMIN_IP in Railway variables.
...
async function initDb() {
  ...
  await run(`CREATE TABLE IF NOT EXISTS streams (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER,
  ...
  if (!admin) {
    ...
    await run(
      "INSERT INTO users (username, password_hash, is_admin, channel_name) VALUES (?, ?, 1, ?)",
...
async function fetchJson(url) {
  const r = await fetch(url, { headers: { "User-Agent": "schedule-app" } });
...
async function youtubeLookup(channelUrl) {
  ...
  if (YOUTUBE_API_KEY) {
    try {
      ...
      if (parsed.type === "handle") {
        const url = `https://www.googleapis.com/youtube/v3/channels?part=snippet&forHandle=${encodeURIComponent(parsed.value)}&key=${YOUTUBE_API_KEY}`;
        const data = await fetchJson(url);
        channelId = data.items?.[0]?.id || "";
        result.channelName = data.items?.[0]?.snippet?.title || "";
        result.avatarUrl = data.items?.[0]?.snippet?.thumbnails?.default?.url || "";
      }
      if (!channelId && parsed.value) {
        const search = await fetchJson(`https://www.googleapis.com/youtube/v3/search?part=snippet&type=channel&maxResults=1&q=${encodeURIComponent(parsed.value)}&key=${YOUTUBE_API_KEY}`);
        channelId = search.items?.[0]?.snippet?.channelId || "";
        result.channelName = search.items?.[0]?.snippet?.channelTitle || "";
      }

      if (channelId) {
        const upcoming = await fetchJson(`https://www.googleapis.com/youtube/v3/search?part=snippet&channelId=${encodeURIComponent(channelId)}&eventType=upcoming&type=video&order=date&maxResults=1&key=${YOUTUBE_API_KEY}`);
        const item = upcoming.items?.[0];
        if (item) {
          result.title = item.snippet?.title || "Scheduled Stream";
          result.thumbnailUrl = item.snippet?.thumbnails?.high?.url || item.snippet?.thumbnails?.medium?.url || "";
          result.videoUrl = `https://www.youtube.com/watch?v=${item.id.videoId}`;
          result.channelUrl = `https://www.youtube.com/channel/${channelId}`;
          result.source = "youtube_api";

          const video = await fetchJson(`https://www.googleapis.com/youtube/v3/videos?part=liveStreamingDetails,snippet&id=${item.id.videoId}&key=${YOUTUBE_API_KEY}`);
          result.scheduledAt = video.items?.[0]?.liveStreamingDetails?.scheduledStartTime || "";
        }
      }
    } catch (e) {
  ...
  if (!result.thumbnailUrl) {
    try {
      ...
      result.source = result.source === "none" ? "youtube_oembed" : result.source;
    } catch (e) {
      result.source = result.source === "none" ? "embed_unavailable" : result.source;
    }
  }
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    const out = await run(
      "INSERT INTO users (username, password_hash, channel_name, channel_url) VALUES (?, ?, ?, ?)",
      [username, hash, channelName, channelUrl]
    );
...
app.put("/api/profile", requireAuth(async (req, res) => {
  await run(
    "UPDATE users SET channel_name = ?, channel_url = ?, bio = ?, avatar_url = ? WHERE id = ?",
    [
      safeString(req.body.channelName),
      safeString(req.body.channelUrl),
      safeString(req.body.bio),
      safeString(req.body.avatarUrl),
      req.user.id,
    ]
  );
  broadcast();
...
app.get("/api/streams", async (req, res) => {
  ...
  const mapped = rows
    .filter((r) => user?.is_admin || r.status === "approved" || r.user_id === user?.id)
...
app.post("/api/streams", requireAuth(async (req, res) => {
  ...
  await run(
    `INSERT INTO streams (user_id, channel_name, channel_url, title, scheduled_at, duration_minutes, thumbnail_url, youtube_video_url, status)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')`,
    [req.user.id, channelName, channelUrl, title, scheduledAt, duration, thumbnailUrl, videoUrl]
  );
  broadcast();
  res.json({ ok: true });
}));
...
app.put("/api/streams/:id", requireAuth(async (req, res) => {
  ...
  await run(
    `UPDATE streams SET channel_name=?, channel_url=?, title=?, scheduled_at=?, duration_minutes=?, thumbnail_url=?, youtube_video_url=?, status=?, updated_at=CURRENT_TIMESTAMP WHERE id=?`,
    [
      safeString(req.body.channelName, stream.channel_name),
      safeString(req.body.channelUrl, stream.channel_url),
      safeString(req.body.title, stream.title),
      safeString(req.body.scheduledAt, stream.scheduled_at),
      Number(req.body.durationMinutes || stream.duration_minutes),
      safeString(req.body.thumbnailUrl, stream.thumbnail_url),
      safeString(req.body.youtubeVideoUrl, stream.youtube_video_url),
      status,
      id,
    ]
  );
...
<title>YouTube Stream Schedule</title>
<style>
  ...
  <div class="grid">
    <section class="panel">
      <div class="tabs">
        ...
        <button class="tab" data-view="submit">Submit Stream</button>
        <button class="tab" data-view="profile">Profile</button>
        <button class="tab adminTab hidden" data-view="admin">Admin Approval</button>
        <button class="tab adminTab hidden" data-view="setup">Setup / YouTube Lookup</button>
      </div>
      ...
      <div id="view-submit" class="view hidden">
        ...
        <form id="streamForm" class="stack hidden">
          <div class="row">
            <div><label>Channel Name</label><input id="sChannelName" required /></div>
            <div><label>Channel URL</label><input id="sChannelUrl" required placeholder="https://youtube.com/@name" /></div>
          </div>
          <div><label>Stream Title</label><input id="sTitle" required placeholder="Tonight's show" /></div>
          <div class="row">
            <div><label>Scheduled Date/Time</label><input id="sTime" type="datetime-local" required /></div>
            <div><label>Duration Minutes</label><input id="sDuration" type="number" value="120" min="15" /></div>
          </div>
          <div class="row">
            <div><label>Thumbnail URL</label><input id="sThumb" placeholder="Optional" /></div>
            <div><label>YouTube Video URL</label><input id="sVideo" placeholder="Optional" /></div>
          </div>
      ...
      <div id="view-profile" class="view hidden">
        ...
        <form id="profileForm" class="stack hidden">
          <div class="row">
            <div><label>Channel Name</label><input id="pChannelName" /></div>
            <div><label>Channel URL</label><input id="pChannelUrl" /></div>
          </div>
      ...
      <div id="view-setup" class="view hidden">
        ...
        <div class="panel adminOnly">
          <p class="mini">Paste a channel URL. The server tries the YouTube API first if YOUTUBE_API_KEY is set. If not, it attempts public embed/oEmbed data.</p>
          <div class="stack">
            <label>Streamer Channel URL</label>
            <input id="lookupUrl" placeholder="https://youtube.com/@creator" />
            <button id="lookupBtn" class="blue">Auto-Lookup Scheduled Stream</button>
    ...
    <aside class="panel">
      ...
      <div id="loggedOut" class="stack">
        <div class="notice">Admin can log in with username <b>admin</b>. Password is the one configured on the server.</div>
...
function api(path, options={}) {
  return fetch(path, { credentials:'include', headers:{'Content-Type':'application/json'}, ...options }).then(async r => {
...
function defaultThumb(s) {
  return s.thumbnail_url || 'https://dummyimage.com/1280x720/202033/ffffff&text=YouTube+Stream';
}

function renderStream(s, admin=false) {
  const status = s.computed_status || s.status;
  const link = s.youtube_video_url || s.channel_url;
  return `<div class="stream">
    <img class="thumb" src="${esc(defaultThumb(s))}" onerror="this.src='https://dummyimage.com/1280x720/202033/ffffff&text=Stream'" />
    <div>
      <span class="badge ${esc(status)}">${esc(status)}</span>
      <h3>${esc(s.title)}</h3>
      <div class="meta"><b>${esc(s.channel_name)}</b> • ${fmtTime(s.scheduled_at)} • ${esc(s.duration_minutes)} min</div>
      <div class="meta">${esc(s.bio || '')}</div>
      <div class="actions">
        <a href="${esc(link)}" target="_blank"><button class="secondary" type="button">Open Stream</button></a>
        ${admin ? `<button class="good" onclick="approve(${s.id})">Approve</button><button class="warn" onclick="rejectStream(${s.id})">Reject</button><button class="blue" onclick="editStream(${s.id})">Edit</button><button class="secondary" onclick="deleteStream(${s.id})">Delete</button>` : ''}
      </div>
    </div>
  </div>`;
}
...
function render() {
  ...
  $('liveList').innerHTML = approved.filter(s => s.computed_status === 'live').map(s => renderStream(s)).join('') || '<div class="notice">Nobody is marked live right now.</div>';
  $('upcomingList').innerHTML = approved.filter(s => s.computed_status === 'upcoming').map(s => renderStream(s)).join('') || '<div class="notice">No approved upcoming streams yet.</div>';
  $('endedList').innerHTML = approved.filter(s => s.computed_status === 'ended').slice(-5).map(s => renderStream(s)).join('') || '<div class="notice">No ended streams yet.</div>';

  if (me?.is_admin) {
    $('adminList').innerHTML = streams.filter(s => s.status !== 'approved').map(s => renderStream(s, true)).join('') || '<div class="notice">No pending streams.</div>';
  }
}
...
async function load() {
  ...
  me = meData.user;
  document.querySelectorAll('.adminTab').forEach(x => x.classList.toggle('hidden', !me?.is_admin));
...
async function editStream(id){
  ...
  await api('/api/streams/'+id,{method:'PUT',body:JSON.stringify({
    channelName:s.channel_name, channelUrl:s.channel_url, title, scheduledAt,
    durationMinutes:s.duration_minutes, thumbnailUrl:s.thumbnail_url,
    youtubeVideoUrl:s.youtube_video_url, status
  })});
  await load();
}
...
$('loginForm').addEventListener('submit', async e => {
  e.preventDefault();
  try { await api('/api/login',{method:'POST',body:JSON.stringify({username:$('loginUser').value,password:$('loginPass').value})}); await load(); }
  catch(err){ alert(err.message); }
});

$('registerForm').addEventListener('submit', async e => {
  e.preventDefault();
  try { await api('/api/register',{method:'POST',body:JSON.stringify({username:$('regUser').value,password:$('regPass').value,channelName:$('regChannelName').value,channelUrl:$('regChannelUrl').value})}); await load(); }
  catch(err){ alert(err.message); }
});
...
$('profileForm').addEventListener('submit', async e => {
  ...
  alert('Profile saved.'); await load();
});
...
$('lookupBtn').addEventListener('click', async () => {
  ...
  try {
    const data = await api('/api/youtube/lookup',{method:'POST',body:JSON.stringify({channelUrl:$('lookupUrl').value})});
...
try {
  ...
  es.onerror = () => { $('connectionStatus').textContent = 'Polling backup active'; };
} catch(e) {}
...
initDb().then(() => {
  app.listen(PORT, () => console.log(`Schedule app running on ${BASE_URL}`));
});
